...
ProductionAllocation ProductionAllocationRepository::save(ProductionAllocation allocation) {
	...
	query.prepare(R"(
        INSERT INTO production_allocations (optimization_config_id, order_id, day, units)
        VALUES (:optimization_config_id, :order_id, :day, :units)
    )");
...
std::vector<ProductionAllocation> ProductionAllocationRepository::all() const {
	...
	QSqlQuery query(Database::instance().connection());
	if (!query.exec("SELECT id, optimization_config_id, order_id, day, units FROM production_allocations"))
...
std::vector<ProductionAllocation> ProductionAllocationRepository::by_order(int order_id) const {
	std::vector<ProductionAllocation> allocations;
	QSqlQuery query(Database::instance().connection());
	query.prepare("SELECT id, optimization_config_id, order_id, day, units FROM production_allocations WHERE order_id = :order_id");
